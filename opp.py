import streamlit as st
import pandas as pd
import yfinance as yf
import plotly.graph_objects as go
from datetime import datetime
import numpy as np

# ==========================================
# 1. 페이지 설정 및 로컬 데이터 유지 (Session State)
# ==========================================
st.set_page_config(page_title="UPRO 실전 매매 터미널", page_icon="🏦", layout="wide")

# [로컬 저장 로직] 처음 앱에 접속했을 때 기본값 설정
if 'seed' not in st.session_state: st.session_state.seed = 37000.0
if 'qty' not in st.session_state: st.session_state.qty = 77
if 'avg' not in st.session_state: st.session_state.avg = 115.76
if 'step' not in st.session_state: st.session_state.step = 2

# 사이드바: 입력창 (세션 상태와 연결)
st.sidebar.markdown("### 🏦 나의 계좌 정보 업데이트")

seed = st.sidebar.number_input("1. 총 투자 원금 ($)", value=st.session_state.seed, step=100.0)
qty = st.sidebar.number_input("2. 현재 보유 수량 (주)", value=st.session_state.qty, step=1)
avg = st.sidebar.number_input("3. 나의 현재 평단가 ($)", value=st.session_state.avg, step=0.01)
step = st.sidebar.select_slider("4. 다음 매수 회차 선택", options=[1, 2, 3], value=st.session_state.step)

# 사용자가 입력한 값을 세션에 즉시 저장 (새로고침 시 유지됨)
st.session_state.seed = seed
st.session_state.qty = qty
st.session_state.avg = avg
st.session_state.step = step

# 전략 고정 변수
TICKER = "UPRO"
N_SIGMA, BUY_MULT, SELL_MULT = 2, 0.85, 0.35
WEIGHTS = [1, 1, 2]

# ==========================================
# 2. 데이터 수집 함수
# ==========================================
@st.cache_data(ttl=600)
def get_market_data():
    tickers = [TICKER, "USDKRW=X"]
    try:
        raw_data = yf.download(tickers, period="30d", progress=False)['Close']
        if raw_data.empty: return None
        return raw_data.dropna()
    except: return None

data = get_market_data()

# ==========================================
# 3. 실시간 계산 및 화면 출력
# ==========================================
if data is not None and not data.empty and len(data) >= 2:
    # 기초 데이터
    last_close_usd = float(data[TICKER].iloc[-1])
    exchange_rate = float(data['USDKRW=X'].iloc[-1])
    
    # 자산 및 수익 계산
    used_cash_usd = qty * avg
    remaining_cash_usd = seed - used_cash_usd
    current_eval_usd = qty * last_close_usd
    profit_loss_usd = current_eval_usd - used_cash_usd
    profit_loss_krw = profit_loss_usd * exchange_rate # 원화 수익금
    return_rate_usd = (profit_loss_usd / used_cash_usd * 100) if used_cash_usd > 0 else 0
    
    # LOC 계산
    returns = data[TICKER].pct_change().dropna()
    sigma = returns.tail(N_SIGMA).std() if len(returns) >= N_SIGMA else 0
    buy_loc_usd = last_close_usd * (1 + BUY_MULT * sigma)
    sell_loc_usd = last_close_usd * (1 + SELL_MULT * sigma)
    
    target_step_usd = seed * (WEIGHTS[step-1] / sum(WEIGHTS))
    buy_qty = int(min(target_step_usd, remaining_cash_usd) / buy_loc_usd) if buy_loc_usd > 0 else 0

    # --- [효과] 수익 축하 로직 (10만원 이상) ---
    if profit_loss_krw >= 100000:
        st.balloons() # 풍선 효과
        st.markdown("""
            <style>
            .stApp { border: 8px solid #FFD700; } /* 황금 테두리 */
            </style>
            """, unsafe_allow_html=True)
        st.success(f"🎊 축하합니다! 원화 수익 **{profit_loss_krw:,.0f}원** 달성! 목표를 향해 순항 중입니다! 🎊")

    # --- 메인 화면 구성 ---
    st.title("📟 UPRO 실전 매매 터미널")
    st.caption(f"최종 업데이트: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} (환율: {exchange_rate:,.2f}원)")

    # 1. 최상단 주문 가이드 (가장 중요)
    st.divider()
    o1, o2 = st.columns(2)
    with o1:
        st.markdown(f"""
        <div style="background-color:rgba(255, 75, 75, 0.1); padding:20px; border-radius:10px; border-left: 8px solid #FF4B4B;">
            <h3 style="color:#FF4B4B; margin:0;">🔵 매수 LOC (Step {step})</h3>
            <h1 style="margin:10px 0;">${buy_loc_usd:.2f}</h1>
            <h4 style="margin:0;">주문 수량: {buy_qty}주 <span style="font-size:14px; color:gray;">(약 {buy_loc_usd*exchange_rate:,.0f}원)</span></h4>
        </div>
        """, unsafe_allow_html=True)
    with o2:
        st.markdown(f"""
        <div style="background-color:rgba(27, 107, 255, 0.1); padding:20px; border-radius:10px; border-left: 8px solid #1B6BFF;">
            <h3 style="color:#1B6BFF; margin:0;">🔴 매도 LOC (전량)</h3>
            <h1 style="margin:10px 0;">${sell_loc_usd:.2f}</h1>
            <h4 style="margin:0;">주문 수량: {qty}주 <span style="font-size:14px; color:gray;">(약 {sell_loc_usd*exchange_rate:,.0f}원)</span></h4>
        </div>
        """, unsafe_allow_html=True)

    # 2. 자산 현황 섹션
    st.write("")
    c1, c2, c3 = st.columns(3)
    c1.metric("내 평단가", f"${avg:,.2f}", f"{avg*exchange_rate:,.0f}원", delta_color="off")
    # 수익률 컬러 적용
    p_color = "normal" if profit_loss_krw >= 0 else "inverse"
    c2.metric("원화 수익금", f"{profit_loss_krw:+,.0f}원", f"{return_rate_usd:+.2f}%", delta_color=p_color)
    c3.metric("남은 현금", f"${remaining_cash_usd:,.2f}", f"{remaining_cash_usd*exchange_rate:,.0f}원", delta_color="off")

    # 3. 그래프 섹션 (우측 라벨 가독성 강화)
    st.divider()
    st.subheader("📈 실시간 가격 가이드라인")

    fig = go.Figure()

    # 주가 선
    fig.add_trace(go.Scatter(
        x=data.index[-15:], y=data[TICKER].tail(15), 
        mode='lines+markers', name='현재가',
        line=dict(color='#00FF00', width=2)
    ))

    # 가이드라인 설정
    lines = [
        {"y": sell_loc_usd, "color": "#1B6BFF", "text": "매도 LOC"},
        {"y": avg, "color": "white", "text": "내 평단가"},
        {"y": buy_loc_usd, "color": "#FF4B4B", "text": "매수 LOC"}
    ]

    for line in lines:
        fig.add_hline(y=line["y"], line_dash="dot", line_color=line["color"], line_width=2)
        fig.add_annotation(
            x=1.02, y=line["y"], xref="paper", yref="y",
            text=f"<b>{line['text']}<br>${line['y']:.2f}</b>",
            showarrow=False, font=dict(size=13, color=line["color"]),
            align="left", xanchor="left"
        )

    fig.update_layout(
        template="plotly_dark", height=550,
        margin=dict(l=10, r=120, t=50, b=10),
        xaxis=dict(showgrid=True, gridcolor='gray', tickformat='%m-%d'),
        yaxis=dict(showgrid=True, gridcolor='gray', side="left"),
        showlegend=False
    )
    
    st.plotly_chart(fig, use_container_width=True)

else:
    st.error("시장이 닫혀있거나 데이터를 가져올 수 없습니다.")